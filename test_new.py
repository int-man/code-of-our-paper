...
def evaluation(eva_csv_name):
    ...
    while index < len(original_results):
        ...
        while index < len(original_results) and original_results["incident_id"][index] == original_results["incident_id"][index-1]:
            ...
            #print(original_results["incident_id"][index - 1])
            index += 1
        Same_Incident_List.append(original_results["id"][index-1])
        #print(original_results["incident_id"][index-1])
        id_index = 0
        ...
        while id_index < len(Same_Incident_List)-1:
            ...
            id_index += 1
        if flag == 1:
            AlertNumber_In_Wrong_Incident += len(Same_Incident_List)

        if id_index >= len(Same_Incident_List)-1 and len(Same_Incident_List)>1:
            Correct_Incident_Number += 1
        if id_index >= len(Same_Incident_List)-1 and len(Same_Incident_List) >= 1:#独立
...
print(VCR_list)
print(thread_list)
print(np.mean(ACR_list))
print(np.mean(VCR_list))
